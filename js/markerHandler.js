...
AFRAME.registerComponent("markerhandler", {
    init: async function () {
        ...
        this.el.addEventListener("markerFound", () => {
          ...
          modelList[barcodeValue]["models"].map(item => {
    ...
    placeTheModel: function(modelName, models){
        var isListContainModel = this.isModelPresentInArray(modelList, moelName)
    ...
    tick: async function () {
        if (modelList.length > 1){
            ...
            var messageText = document.querySelector("#mesage-text")
